...
exports.view = (function() {
        ...
        this._click = setTimeout(
            function() {
                var button = currentTouches.length == 2
                    ? 3
                    : 1;
                self.parentEl.dispatchEvent(new CustomEvent(
                    "buttondown",
                    {"detail": button}));
                self.parentEl.dispatchEvent(new CustomEvent(
                    "buttonup",
    ...
    exports.onloadCallbacks.push(function() {
        ...
        for (var i = 0; i < els.length; i++) {
            ...
            var onbuttondown = el.getAttribute("onbuttondown");
            if (onbuttondown) {
                el.addEventListener("buttondown", function(event) {
                    eval(onbuttondown);
                })
            }
            var onbuttonup = el.getAttribute("onbuttonup");
            if (onbuttonup) {
                el.addEventListener("buttonup", function(event) {
                    eval(onbuttonup);
                })
            }
            var onmove = el.getAttribute("onmove");
            if (onmove) {
                el.addEventListener("move", function(event) {
                    eval(onmove);
                })
            }
            var onscroll = el.getAttribute("onscroll");
            if (onscroll) {
