...
exports.app = (function() {
        ...
        onClose: function() {
            ...
            if (document.body.classList.contains("error")) {
                messagebox.show(_(
                    ...
                    "Failed to connect. If you have turned <em>Require QR code "
                    + "scan for connections</em> on, please click <em>Connect "
                    + "mobile device...</em> in the main menu and scan the QR "
                    + "code to connect."),
                    ["error"]);
            }
            else {
    ...
    addEventListener("load", function() {
        ...
        ws = new WebSocket(
            "ws://${host}/controller".format(
                    util.mapSupplier(document.location))
                + (document.location.hash
                        ? "?access_token=" + document.location.hash.substring(1)
                        : ""));
